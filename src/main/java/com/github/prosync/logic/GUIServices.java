...
/**
 * Created by huseby on 11/27/14. Class GUIServices, Created to execute all
 * inner commands a GUI normally would.
 */
public final class GUIServices {
     ...
     *
     * @return returns a ArrayList with NetworkInterfaces that are up and not
     * virtual.
    ...
    public static NetworkInterface findInterface(String displayName) throws SocketException {
        ...
        for (NetworkInterface ni : cc.getConnectedWIFINIS()) {
            if (ni.getDisplayName().equals(displayName)) {
                return ni;
            }
        }
        return null;
    ...
    public static ArrayList<String> getDownloadableFiles() {
        ...
        ArrayList<String> list = new ArrayList<>();
        for (String s : cc.getFileListVideo(url)) {
            list.add(s);
        }
        for (String s : cc.getFileListSingleShot(url)) {
            list.add(s);
        }
        for (String s : cc.getFileListBurst(url)) {
            list.add(s);
        }
        return list;
    }
    ...
    public static String getMode(String name) {
        ...
        try {
            if (matcherBurst.find()) {
                return "Burst";
            }
            if (matcherSingleShot.find()) {
                return "Single";
            }
